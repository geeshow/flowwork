...
- **API 콜렉션도 같은 브랜치 플로우** — API 콜렉션 화면(`/apic`)은 develop 읽기 전용으로 시작하고, `/apic/b/{branch}`로 수정 모드 진입 시 콜렉션 편집·가져오기·삭제가 활성화. 콜렉션 트리에 상태 배지, 커밋·푸시·머지 바 공용. 편집 브랜치의 워크플로우 편집/실행은 그 브랜치의 콜렉션(카탈로그·환경변수)을 그대로 쓴다
...
- **동일 브랜치 동시 저장 충돌 감지 (낙관적 잠금)** — 조회 시 파일 해시 버전을 받고 저장 시 검사. 그 사이 다른 사용자가 저장/삭제했으면 409로 거부하고, 편집기가 서버 최신 내용과 내 수정을 좌우 diff로 보여준다 → "서버 최신 불러오기" 또는 "덮어쓰기 저장"으로 해결
...
| API 콜렉션 브랜치 편집 — /apic 브랜치 플로우, 콜렉션 상태 배지, 브랜치별 카탈로그 | ✅ |
